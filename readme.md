...
Модели не имею прямого доступа к своим свойствам, их можно получить только через методы,
такой подход избавляет случайное изменение сущности.

###### Контроллеры.
Они же проводники - посредники между внешним и миром backend.
Контроллеры имеют прямой доступ к сущностям и репозиториям, контроллеры занимаются проверкой качеством и целостностью данных и сущностей.
###### Роутер.
Это просто маршруты, они определяют какому контроллеру был адресован запрос и какие данные требует контроллер.

Работа логике на примере создания нового клиента.

POST запрос, маршрут вызывает нужный контроллер для добавления клиента в базу.
Передает данные о новом клиенте имя, страну, город и требуемую валюту.
После контроллер создает экземпляр сущности клиента, отдает ей получение данные,
...
Репозиторий забирает данные нужные для создания клиента из сущности и передает их в базу данных.

Хранимая Процедура базы для создания клиента, принимает данные. Первым делом происходит поиск валюты в базе, что была указана при создании клиента, если такая валюта у нас есть (такой алиас) то просто крепим пользователю id наше валюты, если нет, то валюта будет создана и прикреплена. После этого идет генерация уникального uid для клиента (некий личный счет). Далее когда все данные готовы, происходит запись данных о клиенте. Назад в репозиторий возвращается свежо созданный счет (uid) клиента и статус (определяющих успех или провал операции).
Репозиторий получив данные от процедуры, добавляет их в сущность клиента и возвращает полноценную сущность контроллеру.
Контроллер забирает нужные данные из клиентской сущностей и отдает на выход в json формате (с 201 заголовком).
...
- Вся логика для работы с денюшками, валютой скрыта в базе, а это стабильно, быстро.
- Может выдерживать большую нагрузку (магии почти нет).
- Стандарт JSON позволяет использовать API даже на холодильнике.

###### Минусы:
- Иза отсутствие ORM очень болезненно будет переезд на другою базу данных.
- Даже не знаю что еще...

##### Архитектура базы данных.
Проста как три копейки, объекты и действия над объектами.
Например валюта хранится в отдельной таблицы, можно прикреплять всем (many - to - one),
Действие для счетов пользователя тоже хранятся отдельно, любой лог просто ссылается на действие которое он совершил. (many - to - one)
